...
public class Jsonparser {
	...
	// Differents états du parser
	public enum StateWorking {work, doNothing}
	public enum StateCurrent {idle, endWord, end}
	...
	public String[] getTabActions(String plan) {
		...
		StateWorking sw = StateWorking.work;
		StateCurrent sc = StateCurrent.idle;
		
		while(i<plan.length() && !sc.equals(StateCurrent.end)) {
			str = "";
			sc = StateCurrent.idle;
			while(sc.equals(StateCurrent.idle)) {
				...
				case '"' :
					break;
				case '[' :
					sw = StateWorking.doNothing;
					break;
				case ']' :
					sw = StateWorking.work;
					break;
				case ':':
					str = str+" ";
					break;
				case ',':
					if (niveau == 1 && sw.equals(StateWorking.work)) sc = StateCurrent.endWord;
					else str = str+" ";
					break;
				case '}':
					niveau --;
					if(niveau == 0) sc = StateCurrent.end;
					break;
				default :
					str = str+c;
				}
			...
			if (getPremierMot(str).equals("timeSpecifiers")) {
				nbactions = findNbActions(str);
			} else {
				listString.add(str);
			}
		}
		return actionDansLOrdre(listString, nbactions);
	}
